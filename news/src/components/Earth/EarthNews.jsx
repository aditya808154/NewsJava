...
const EarthNews = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await fetch("http://localhost:8080/api/earth/popular");
        const data = await response.json();
        
        // Latest data first (Reverse logic)
        const rawData = data.earthNews || [];
        setEarthData([...rawData].reverse()); 
      } catch (error) {
        console.error("Error fetching Earth news:", error);
      }
    };
  ...
  // Card UI sync: 1-line text style
  const oneLineStyle = {
    display: "-webkit-box",
    WebkitLineClamp: "1",
  ...
  const settings = {
    infinite: earthData.length > 1,
    speed: 500,
    slidesToShow: 2, // Default desktop view balanced
    slidesToScroll: 1,
    arrows: false,
    autoplay: earthData.length > 1,
    autoplaySpeed: 3000,
    responsive: [
      {
        breakpoint: 1024,
